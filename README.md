...
SciWhisper — бесплатный локальный компилятор устной научной нотации. Whisper распознаёт речь, а SciWhisper превращает текст в химические формулы, математические выражения и физические обозначения.
...
Аудио и научный текст не отправляются в облако. Если локальной модели нет, SciWhisper завершает операцию с ошибкой и не пытается скачать её автоматически.

## Скачать готовые сборки

Для GitHub Release подготовлены ровно два файла в локальной папке `release-assets/`:

| Система | Файл для прикрепления к GitHub Release | Как запустить |
|---|---|---|
...
Архивы намеренно исключены из Git-индекса: их нужно прикреплять к странице GitHub Release, а не хранить в истории исходного кода.
...
![Макет Windows-настроек Si-Witch](docs/images/si-witch-windows-settings-mockup.png)
